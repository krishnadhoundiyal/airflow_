...
# Define the DAG
dag = DAG(dag_id="workflow_21", default_args=default_args, schedule_interval="13 01 * * 1,2,6", catchup=False)
...
# Define Branch task to check the state of the most recent task
branch_task = BranchPythonOperator(
    task_id='branch_task',
    python_callable=python_callable_check_task,
    op_kwargs={ 'dag_id': "workflow_21", 'task_id': "python_task" },
    provide_context=True,
    dag=dag
)

# Trigger DAG on success
trigger_on_success = TriggerDagRunOperator(
    task_id='trigger_on_success',
    trigger_dag_id='workflow_3',
    conf={
        "Protocol": "SFTP",
        "Host Name": "10.0.0.1",
        "Port": "22",
        "UserName": "hello",
        "Password": "world",
        "Source Location": "/mnt/hello",
        "Destination Location": "/mnt/pass",
        "File Naming Convention": "abc*.txt",
        "Delete Files From Source SFTP Location": "1",
    },
     dag=dag
 )

trigger_on_failure = TriggerDagRunOperator(
    task_id='trigger_on_failure',
    trigger_dag_id='workflow_4',
    dag=dag
)

# Define the task flow

python_task >> branch_task >> [trigger_on_success, trigger_on_failure]
